...
namespace MobileVisit.Tests
{
    public abstract class MobileVisitTestBase : AbpIntegratedTestBase<MobileVisitTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MobileVisitTestBase()
        {
        ...
        protected void UsingDbContext(Action<MobileVisitDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<MobileVisitDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MobileVisitDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MobileVisitDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MobileVisitDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MobileVisitDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<MobileVisitDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MobileVisitDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MobileVisitDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MobileVisitDbContext, Task<T>> func)
        {
            T result;
